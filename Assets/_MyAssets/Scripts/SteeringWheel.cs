...
public class SteeringWheel : MonoBehaviour {
    ...
    void Update() {
        ...
        if (!wheelBeingHeld && !Mathf.Approximately(0f, wheelAngle)) {
            ...
            //debugText2.SetText("wheelAngleClamped:" + GetClampedValue().ToString());
        }
        car.transform.Translate(new Vector2(GetClampedValue(), 0) * Time.deltaTime * carSpeed);
        if (car.transform.position.x >= 1.75f) {
            car.transform.position = new Vector2(1.75f, car.transform.position.y);
        } else if (car.transform.position.x <= -1.75f) {
            car.transform.position = new Vector2(-1.75f, car.transform.position.y);
        }

        debugText.SetText("wheelAngle:" + (wheelAngle / maximumSteeringAngle).ToString());
